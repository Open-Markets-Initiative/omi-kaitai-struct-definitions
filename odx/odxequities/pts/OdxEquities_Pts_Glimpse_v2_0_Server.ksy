...
meta:
  id: odx_odxequities_pts_glimpse_v2_0_server
...
types:
  ...
  short_selling_price_restriction_state_message:
    seq:
      ...
      - id: short_selling_state
        ...
        doc: 'Current short selling price restriction status'
  order_added_message:
    seq:
      - id: nanoseconds
        type: nanosecond_offset
      ...
      - id: order_number
        type: u8
        doc: 'Reference number of the accepted order Zero indicates a reference price update'
      - id: buy_sell_indicator
        type: u1
        enum: buy_sell_indicator
        doc: 'Side of the order Ignore if reference price update'
      - id: quantity
        type: u4
        doc: 'Total number of shares added to the book Ignore if reference price update'
      - id: orderbook_id
        type: str
        size: 4
        encoding: ASCII
      ...
      - id: price
        type: decimal_u4_1
        doc: 'Price of the order For a reference price update, a value of 214,748,364.7 (7FFFFFFF hex) denotes no reference price available. Implied decimal with scale 1e-1'
  ...
  end_of_snapshot_message:
    seq:
      - id: sequence_number
        type: u8
        doc: 'Itch market data feed sequence number'
  second_timestamp:
    seq:
      - id: time
        type: s4
