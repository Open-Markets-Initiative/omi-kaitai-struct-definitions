# ---------------------------------------------------------------------
# Kaitai struct definition for: Nyse ArcaOptions ComplexFeed Pillar v1.0.h
#
# Protocol:
...
meta:
  id: nyse_arcaoptions_complexfeed_pillar_v1_0_h
  title: Nyse ArcaOptions ComplexFeed Pillar v1.0.h
  license: GPL-3.0
  endian: le

doc: 'New York Stock Exchange Nyse Arca Options Complex Feed Pillar v1.0.h'
...
types:
  ...
  options_quote_message:
    seq:
      ...
      - id: ask_price
        type: s4
        doc: 'The Ask price. A positive value is implicit (no sign), a negative value is indicated by a negative sign. Use the Price scale from the Series Index message'
      - id: ask_volume
        type: u4
        doc: 'Total quantity available at the above Ask price'
      - id: bid_price
        type: s4
        doc: 'The Bid price. A positive value is implicit (no sign), a negative value is indicated by a negative sign. Use the Price scale from the Series Index message'
      - id: bid_volume
        type: u4
  ...
  options_trade_message:
    seq:
      ...
      - id: trade_price
        type: s4
        doc: 'Trade price. A positive value is implicit (no sign), a negative value is indicated by a negative sign. Use the Price scale from the series index mapping'
      - id: volume
        type: u4
  ...
  series_rfq_message:
    seq:
      ...
      - id: type_field
        type: u1
        enum: type_field
        doc: 'Order Type of CUBE/COA/BOLD'
      - id: capacity
        type: u1
        enum: capacity
        doc: 'Customer or Firm capacity specified with the order. Only used for BOLD (Type = B)'
      - id: total_quantity
        type: u4
        doc: 'Total quantity'
      - id: working_price
        type: s4
        doc: 'RFQ Price for CUBE/COA'
      - id: participant
        type: u4
        doc: 'OCC number for the Clearing firm specified with the order. 0 if no OCC number was specified. Only used for BOLD (Type = B)'
      - id: auction_id
        type: u8
      ...
      - id: rfq_status
        type: u1
        enum: rfq_status
        doc: 'Status field for CUBE/COA (Type = C)'
...
enums:
  ...
  message_type:
    ...
    320:
      ...
      doc: 'Used to publish the complex Last Sales.'
    307:
      id: 'series_rfq_message'
      doc: 'Pass-through message from the matching engine published for both Outright and Complex series in the event of a COA auction or a CUBE auction on complex orders.'
  market_id:
    1:
      id: 'nyse_equities'
      doc: 'Nyse Equities'
  ...
  trade_cond_1:
    ...
    0x53:
      id: 'all_outright_series_trades_that_are_generated_as_part_of_an_intermarket_sweep_order'
      doc: 'All Outright Series Trades That Are Generated As Part Of An Intermarket Sweep Order'
    0x44:
      id: 'any_of_the_above_published_after_90_seconds_of_occurring'
