# ---------------------------------------------------------------------
# Kaitai struct definition for: Cboe NeoEquities MulticastFullDepth Pitch v1.0.14
#
# Protocol:
#   Organization: Chicago Board Options Exchange
#   Protocol: Multicast Full Depth
#   Encoding: Pitch
#   Version: 1.0.14
#   Date: 02/09/2026
...
meta:
  id: cboe_neoequities_multicastfulldepth_pitch_v1_0_14
  title: Cboe NeoEquities MulticastFullDepth Pitch v1.0.14
  license: GPL-3.0
  endian: le

doc: 'Chicago Board Options Exchange Cboe Canada NEO Multicast Full Depth Pitch v1.0.14'
...
types:
  ...
  add_order_message:
    seq:
      ...
      - id: order_id
        type: u8
        doc: 'Unique identifier assigned to this order'
      - id: side_indicator
        type: u1
        enum: side_indicator
        doc: 'Order side indicator'
      - id: quantity
        type: u4
        doc: 'Number of shares being added to the order book'
      - id: symbol
        type: str
        size: 14
        encoding: ASCII
      ...
      - id: broker_id
        type: u2
        doc: 'Identity of trading Member that submitted the order (or 1 for anonymous)'
      - id: settlement_type
        type: u1
  ...
  order_executed_message:
    seq:
      ...
      - id: execution_id
        type: u8
        doc: 'Cboe Canada NEO generated day-unique execution identifier of this execution'
      - id: buy_broker_id
        type: u2
  ...
  auction_update_message:
    seq:
      ...
      - id: reference_price
        type: decimal_u8_4
        doc: 'Midpoint of the BBO, or NLSP if entering a delayed closing (trading status ''M''). Not used ($0.00) for Opening Call and reopening call. Implied decimal with scale 1e-4'
      - id: imbalance_quantity
        type: u4
        doc: 'The size of the auction unmatched shares at the Reference Price (for the Closing Call). The size of the auction unmatched shares at the Indicative Price (for the Opening Call and reopening call)'
      - id: imbalance_side_indicator
        type: u1
        enum: imbalance_side_indicator
        doc: 'The side of the auction imbalance at the Reference Price (for the Closing Call). The side of the auction imbalance at the Indicative Price (for the Opening Call and reopening call)'
      - id: matched_shares
        type: u4
        doc: 'The matched shares at the Reference Price (for the Closing Call). The matched shares at the Indicative Price (for the Opening Call and reopening)'
      - id: indicative_price
        type: decimal_u8_4
      ...
      - id: auction_only_price
        type: decimal_u8_4
        doc: 'For the Closing Call: Price at which the Closing Call Book would match using only eligible auction orders. Not used for the Opening Call and reopening call. Implied decimal with scale 1e-4'
      - id: reserved_1
        type: u1
        doc: 'Reserved (undefined)'
...
enums:
  message_type:
    ...
    0x62:
      ...
      doc: 'Represents a newly accepted order on Cboe Canada NEO. It includes a unique Order Id assigned by Cboe Canada NEO to the order.'
    0x63:
      id: 'reduce_size_message'
      doc: 'Sent when a visible order on Cboe Canada NEO is partially reduced. Orders that are reduced in size do not lose priority.'
    0x64:
      id: 'delete_order_message'
      doc: 'Sent whenever a booked order is cancelled or leaves the order book. The Order Id refers to the Order Id of the original Add Order message. An order that is deleted from the book may return to the book later. Therefore, a Delete Order message does not indicate that a given Order Id will not be sent again on a subsequent Add Order message. Members should be prepared to handle this scenario.'
    0x65:
      id: 'modify_order_message'
      doc: 'Sent whenever an open order is visibly modified. The Order Id refers to the Order Id of the original Add Order message. Modify Order messages appearing to be no ops will still lose priority.'
    0x66:
      id: 'order_executed_message'
      doc: 'Sent when a visible order on Cboe Canada NEO is executed in whole or in part. The execution price equals the order price found in the original Add Order message or the order price in the latest Modify Order message referencing the Order Id.'
    0x67:
      id: 'trade_message'
    ...
    0x6b:
      ...
      doc: 'Disseminates price and size information and Composite Market bid and offer prices during Opening Call, Closing Call, and reopening call (halt). Auction Update messages are sent during an auction period, unless in a delayed closing.'
    0x6c:
      id: 'auction_summary_message'
      doc: 'Disseminates the results of an Opening Call, Closing Call, or reopening call (Halt) on Cboe Canada NEO. An Auction Summary message for each Cboe Canada Listed Security is sent at the conclusion of its Opening Call and represents the Cboe official Opening Price (or Closing Price).'
    0x2d:
  ...
  auction_type:
    ...
    0x43:
      ...
      doc: 'Closing Call'
    0x48:
      id: 'halt_auction_reopening_call'
      doc: 'Halt Auction Reopening Call'
  imbalance_side_indicator:
    0x42:
      id: 'buy_side'
      doc: 'Buy Side'
    0x53:
