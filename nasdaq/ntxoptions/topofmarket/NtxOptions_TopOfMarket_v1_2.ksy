# ---------------------------------------------------------------------
# Kaitai struct definition for: Nasdaq NtxOptions TopOfMarket Itch v1.2
#
# Protocol:
...
doc: 'National Association of Securities Dealers Automated Quotations (Nasdaq) Nasdaq Texas Options Top Of Market Itch v1.2'
...
types:
  ...
  timestamp_message:
    seq:
      - id: second
        ...
        doc: 'Number of seconds since midnight. All messages between this Timestamp Message and the next Timestamp Message will have this value of seconds in their timestamp. Seconds since Midnight epoch'
  ...
  options_directory_message:
    seq:
      ...
      - id: minimum_price_variation
        type: u1
        enum: minimum_price_variation
  ...
  security_open_message:
    seq:
      ...
      - id: open_state
        type: u1
        enum: open_state
        doc: 'Reflects the current eligibility for auto execution of the options security in the options market. The allowable values are:'
...
enums:
  message_type:
    0x54:
      ...
      doc: 'For bandwidth efficiency reasons, the timestamp for a message is divided into two pieces: the seconds portion and sub-seconds portion. The seconds portion of the timestamp appears in the Timestamp Message, any message on the stream following this message has this seconds portion of time until the next timestamp message. The format is as follows:'
    0x53:
    ...
    0x71:
      ...
      doc: 'The options system will continuously calculate its best bid and offer position for active options contracts on the options market during the trading day. Whenever the best bid and ask position changes on both sides, the options system will send its best bid and ask update via the data feed for the affected security. A change in bid or ask implies a change in price and/or size. The quote will reflect the highest price displayable in the options system for buy orders/quotes and the lowest price displayable in the options system for sell orders/quotes.'
    0x51:
  ...
  quote_condition:
    0x46:
      id: 'nonfirm_quote'
      doc: 'Nonfirm Quote On Both Bidask Sides'
    0x52:
      id: 'rotational_quote'
