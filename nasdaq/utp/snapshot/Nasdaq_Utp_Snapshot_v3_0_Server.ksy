...
meta:
  id: nasdaq_utp_snapshot_utp_v3_0_server
...
seq:
  - id: server_tcp_packet_header
    type: server_tcp_packet_header_struct
...
types:
  server_tcp_packet_header_struct:
    seq:
      - id: packet_length
        type: u2
      ...
      - id: server_packet_type
        ...
        doc: 'Code identifying the SoupBinTCP packet type sent by the server'
  sequenced_data_packet:
    seq:
      - id: message_header
        type: message_header
        doc: 'UTP Snap-Shot message header — fixed Version + Message Category prefix'
      - id: category_payload
        size: _root.server_tcp_packet_header.packet_length - 3
        type:
  ...
  message_header:
    seq:
      - id: version
        type: u1
        doc: 'UTP Snap-Shot protocol version (1)'
      - id: message_category
        type: str
        size: 1
        encoding: ASCII
        doc: 'UTP Snap-Shot message category: A (Administrative), C (Control), or Q (Quote)'
      ...
      - id: administrative_message_payload
        type:
          ...
          cases:
            ...
            '"F"': enhanced_issue_symbol_directory_message
  ...
  enhanced_issue_symbol_directory_message:
    seq:
      - id: market_center_originator
        type: u1
        enum: market_center_originator
      ...
      - id: etp_indicator
        ...
        size: 1
        encoding: ASCII
        doc: 'Exchange Traded Product (ETP) Indicator'
      - id: new_issue_indicator
        type: str
        size: 1
        encoding: ASCII
        doc: 'New Issue Indicator (IPO status)'
      - id: mpi_tick
        type: u1
        enum: mpi_tick
        doc: 'MPI Tick Rule. All symbols carry Penny MPI until Half-Penny MPI is introduced Nov 2026 per SEC mandate'
      - id: trading_state
        type: u1
        enum: trading_state
        doc: 'Trading State (halted / trading) at the time of dissemination'
      - id: halt_reason
        type: str
        size: 6
        encoding: ASCII
        pad-right: 0x20
        doc: 'Reason for Halt, when Trading State indicates halted'
      - id: reg_sho_action
        type: u1
        enum: reg_sho_action
  ...
  auction_collar_message:
    seq:
      ...
      - id: collar_extension_indicator
        ...
        size: 1
        encoding: ASCII
        doc: 'Collar Extension Indicator. First collar at the pause is 0; increments by 1 for each new collar issued during the pause; resets to 0 on a new pause event'
  snapshot_sequence_message:
    seq:
      - id: market_center_originator
        type: u1
        enum: market_center_originator
      ...
      - id: sequence_number
        type: u8
        doc: 'UTP Data Service quotation-feed sequence number at the time the Snap-Shot was taken. Begin real-time processing from the next sequence'
  control_message:
    seq:
    ...
    seq:
      ...
      - id: quote_message_payload
        type:
          ...
          cases:
            '"D"': utp_combined_quote_message_long_form
            '"B"': odd_lot_quote_message_long_form
  utp_combined_quote_message_long_form:
    seq:
      - id: market_center_originator
        type: u1
        enum: market_center_originator
      ...
      - id: finra_adf_mpid_appendage_indicator
        ...
        doc: 'FINRA ADF MPID Appendage Indicator. Populated with the FINRA ADF update if necessary'
